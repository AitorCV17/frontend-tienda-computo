  ...
  export default defineComponent({
    ...
    setup() {
      ...
      onMounted(async () => {
        ...
        try {
          const { data } = await axios.get(import.meta.env.VITE_BACKEND_URL + '/users/profile', {
            headers: { Authorization: `Bearer ${authStore.token}` }
          })
          nombre.value = data.nombre
          email.value = data.email
        } catch (error: any) {
          notificationMessage.value = 'Error cargando perfil'
        ...
        try {
          await axios.put(
            import.meta.env.VITE_BACKEND_URL + '/users/profile',
            {
              nombre: nombre.value,
              email: email.value,
              password: password.value // si está vacío, backend no lo cambia
            },
            {
              headers: { Authorization: `Bearer ${authStore.token}` }
            }
          ...
          notificationMessage.value = error.response?.data?.message || 'Error al actualizar perfil'
          notificationType.value = 'error'
          notificationIcon.value = 'fa-solid fa-xmark'
        }
      }
